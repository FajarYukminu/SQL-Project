...
--6. Find monthly growth of inventory in percetage breakdown by product categories, ordered by time descendeingly.(time frame Jan 2019 until APR 2022) is there interesting insight?

WITH ct1 AS (
  SELECT DATE_TRUNC(DATE(t1.created_at),MONTH) AS month
    ,t1.product_category AS categories
    ,COUNT (DISTINCT order_id) AS total_orders
    ,CAST(SUM(sale_price) AS INT64) AS total_sales
  FROM `bigquery-public-data.thelook_ecommerce.inventory_items` AS t1
  JOIN `bigquery-public-data.thelook_ecommerce.order_items` AS t2
    ON t1.product_id = t2.product_id
  WHERE status ='Complete' 
  AND t1.created_at BETWEEN '2019-01-01' AND '2022-04-30' 
  GROUP BY 1,2
)
...
, ct2 AS(
  SELECT month
    ,categories
    ,total_orders
    ,LAG(total_orders) OVER(PARTITION BY categories ORDER BY categories,Month)  AS prev_total_orders
    ,total_sales
    ,LAG(total_sales) OVER(PARTITION BY categories ORDER BY categories,Month)  AS prev_total_sales
  FROM ct1
  ORDER BY 2,1
)
...
SELECT Month
  ...
  ,total_orders
  ,prev_total_orders
  ,ROUND ((CAST((total_orders - prev_total_orders) as decimal)/prev_total_orders* 100),2) AS orderpercentchange
  ,total_sales
  ,prev_total_sales
  ,ROUND((CAST((total_sales - prev_total_sales) as decimal)/prev_total_sales* 100),2) AS salespercentchange
FROM ct2
ORDER BY 2, 1 DESC ;
